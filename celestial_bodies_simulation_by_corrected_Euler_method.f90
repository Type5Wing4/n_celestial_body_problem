program celestial_bodies_simluation_by_corrected_Euler_method
        ...
        double precision,allocatable,dimension(:,:) :: x, v, sv, a, f
...
!        if (mod(k,output_interval) .eq. 1) call calc_energy
        ...
        deallocate(body_type,m,x,v,a,f)
...
subroutine calc_velocities_s

        do i = 1, nb_bodies
         sv(i,1) = v(i,1) + F(i,1) * dt / m(i)
         sv(i,2) = v(i,2) + F(i,2) * dt / m(i)
         sv(i,3) = v(i,3) + F(i,3) * dt / m(i)
        end do
...
subroutine calc_next_positions
        ...
        do i = 1, nb_bodies
         x(i,1) = x(i,1) + 0.5d0*(v(i,1)+sv(i,1))*dt
         x(i,2) = x(i,2) + 0.5d0*(v(i,2)+sv(i,2))*dt
         x(i,3) = x(i,3) + 0.5d0*(v(i,3)+sv(i,3))*dt
        end do
